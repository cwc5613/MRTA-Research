...
for iii = 1:length(range)
    ...
    [theta,init_x,init_task] = INIT(n,m);
    ...
    [t,XY] = ode45(@(t,x) odefcn(t,x,n,m,range(iii),theta,alpha,beta),tspan,init_x);
    ...
    %     axis([0 10 0 10])
    %     hold all
    %     %PLOT_ENVIRONMENT(x,y,n,'o',featcolors,task_x3,task_y3,m(iii),'x',argmax_bot,task_type);
    %     m(iii) = PLOT_ENVIRONMENT_MOV(x,y,n,'o',featcolors,task_x3,task_y3,m(iii),'x',argmax_bot,task_type);
    %     hold off
    
    time = toc;
    
    avedist = sum(sum(sqrt(diff(x).^2+diff(y).^2)))/length(x(1,:));
    
    alpha_beta = [alpha_beta;beta/alpha];
    task_count_vec = [task_count_vec;length(x(:,1))/length(TOTALHIT(:,1))];
    het_vec = [het_vec;HETERO_COUNT/length(TOTALHIT(:,1))];
    dist_vec = [dist_vec;avedist];
    range_vec = [range_vec;range(iii)];
    DATA = [alpha_beta,task_count_vec,het_vec,dist_vec,range_vec];
    
    fprintf('(Alpha,Beta): (%2.1f,%2.1f)\n',alpha,beta)
    fprintf('(m,n): (%2.0f,%2.0f)\n',m,n)
    fprintf('RunTime: %2.2f secs\n',time)
    fprintf('Tasks: %2.2f\n',length(TOTALHIT(:,1)))
    fprintf('Timestep/Task: %2.2f\n',length(x(:,1))/length(TOTALHIT(:,1)))
    fprintf('Hetero/Total: %2.2f\n',HETERO_COUNT/length(TOTALHIT(:,1)))
    fprintf('Ave Distance: %2.2f\n',avedist)
    
    fprintf('-----------------------\n')
    
end

function [theta,x0,t0] = INIT(n,m)
...
function Eq = odefcn(t, X, n, m, range, theta,alpha,beta)
