...
for iii = 1:length(alpha)
    for jjj = 1:length(beta)
        ...
        [t,XY] = ode45(@(t,x) odefcn(t,x,n,m,range,theta,alpha(iii),beta(jjj)),tspan,init_x);
        ...
        alpha_beta = [alpha_beta;beta(jjj)/alpha(iii)];
        ...
        DATA = [alpha_beta,task_count_vec,het_vec,dist_vec];
        
        fprintf('(Alpha,Beta): (%2.2f,%2.2f)\n',alpha(iii),beta(jjj))
...
%V0 = [-1 -1; -1 11; 11 11; 11 -1]; %unit square
l = length(V0(:,1));
V0 = [V0; V0(1,:)];
Lines = zeros(l,3);
for i = 1:l
